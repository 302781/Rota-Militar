...
class OnboardingManager {
    ...
    private onboardingData: { [key: string]: any } = {};
    ...
    public selectProfile(value: string, element: HTMLElement): void {
        ...
        if (container) {
            ...
            document.getElementById('profile-next-btn')?.removeAttribute('disabled');
        }
    }

    public selectFitnessLevel(value: string, element: HTMLElement): void {
        const container = element.closest('.option-card-container');
        if (container) {
            container.querySelectorAll('.option-card').forEach(card => card.classList.remove('selected'));
            element.classList.add('selected');
            this.onboardingData.fitnessLevel = value;
            document.getElementById('fitness-next-btn')?.removeAttribute('disabled');
        }
    }

    public toggleObjective(value: string, element: HTMLElement): void {
        ...
        if (this.onboardingData.objectives.length > 0) {
            ...
            document.getElementById('objectives-next-btn')?.setAttribute('disabled', 'true');
        }
    }
    ...
    private showPlanWelcomeScreenBasedOnProfile(): void {
    ...
    public nextCarouselSlide(carouselElement: HTMLElement): void {
        const slides = Array.from(carouselElement.querySelectorAll('.carousel-slide')) as HTMLElement[];
        const dots = Array.from(carouselElement.querySelectorAll('.carousel-dot')) as HTMLElement[];
